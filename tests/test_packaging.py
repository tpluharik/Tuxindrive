...
class PackagingTests(unittest.TestCase):
    ...
    def test_tray_animation_frames_are_valid_and_packaged(self):
        ...
        for frame in range(8):
            ...
            self.assertIn('viewBox="0 0 288 288"', content)
            self.assertIn('r="144"', content)
            self.assertIn("circular activity frame", content)
        for asset in ("packaging/tuxindrive-sync.svg",):
            content = Path(asset).read_text(encoding="utf-8")
            ET.parse(asset)
            self.assertIn('viewBox="0 0 288 288"', content)
            self.assertIn('r="144"', content)
            self.assertIn("circular activity indicator", content)
        error_content = Path("packaging/tuxindrive-error.svg").read_text(encoding="utf-8")
        ET.parse("packaging/tuxindrive-error.svg")
        self.assertIn('viewBox="0 0 288 288"', error_content)
        self.assertIn("circular error indicator", error_content)
        ...
        self.assertIn('"tuxindrive-activity"', app)
        self.assertIn("self.activity_indicator", app)
        self.assertIn("IndicatorStatus.PASSIVE", app)
