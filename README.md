...
Version 0.26.54 correlates checksum failures and cloud duplicate notices by exact provider path, identifies repeated stable metadata/content mismatches as provider-side inconsistencies rather than local edits, and keeps unrelated duplicate warnings separate. It immediately pauses repeated checksum-corrupt downloads, removes incomplete local copies, and reports the exact provider path. It keeps current synchronization activity immediately visible by showing the latest log file and newest records at the top while preserving multi-line errors and tracebacks. It retains safe recovery from abandoned Bisync staging state, bounded provider retries, actionable redacted diagnostics, dedicated Google Drive OAuth credentials, readable dark settings and summary tiles, export-safe Google Workspace verification, system-aware visual layouts, complete Midnight Sync rendering, duplicate-provider detection on both sides, non-destructive automatic cloud-duplicate renaming, verified two-way completion, safe endpoint-change pausing, persistent account renaming, drag-and-drop sidebar ordering, readable multi-line account identities across supported providers and truthful Google Drive root identity with the competitive P0/P1 reliability work and a
user-created, private diagnostic ZIP that redacts credentials and paths by
default; selective-sync presets with a bounded metadata-only count/size
preview; non-destructive streaming-cache recommendations; visible bounded
mount lifecycle phases; and cached runtime provider capability probes that can
only narrow the safe static declarations. A new GTK-free
`tuxindrive-control` command exposes redacted JSON status, capabilities, rule
previews, cache plans and diagnostic-bundle creation for local automation.
Credentialed provider longevity and platform-store tests remain explicit
release gates and are not represented as completed by local simulations.
Version 0.26.38 makes realtime directory moves and deletions resilient: stale
child events are discarded in favor of authoritative reconciliation, vanished
parents no longer poison the retry queue, and incremental error details include
the side, processing phase and source path. It also retains immediate live-log
loading from 0.26.37 and the lower-power monitoring introduced in 0.26.36.
Realtime folders sleep in interruptible kernel
...
tray slots for the stable TuxInDrive logo
and a square, circular activity/error indicator beside it, preventing GNOME
from compressing a wide composite into an oval. Version 0.26.33 made the
animated activity source three times larger. Version 0.26.32 added SessionSifu-style animated tray
activity, a larger adjacent error indicator, and aggregate job-state handling
so concurrent work and outstanding failures remain visible. Version
0.26.31 added restart-aware synchronization scheduling, durable monitor
baseline reuse, incremental Android folder mirroring, shared short-lived
metadata results, cached macOS traffic sampling, and a 50% default bandwidth
reserve for other applications. Invalid or incomplete state still takes the
full safety recovery path. Version 0.26.30 added opt-in bounded local content search, preview-first
cloud-to-cloud copying, categorized recovery advice, managed desktop policy,
private audit export, and reproducible upgrade/reliability/performance evidence.
Content indexing is off by default and excludes files-on-demand mounts; cloud
copy never performs deletions and remains subject to the global bandwidth
controller. Version 0.26.29 added safe automatic recovery from orphaned rclone Bisync locks,
including corrupted locks whose expiry is accidentally set years into the
future. TuxInDrive removes a lock only after its owner process is proven gone
and the private lock file passes freshness, ownership, path, type, and identity
checks. Version 0.26.28 also added immediate **Error details** beside **View log**, including
...
The following controls are enforced in 0.26.54:
...
TuxInDrive requires a dedicated Desktop OAuth application for Google Drive
because rclone's shared Google client is being retired during 2026. Other
browser-OAuth providers may still use rclone defaults for personal
installations, while production or organizational deployments should register
dedicated applications and enter the client ID and secret in the connection
dialog:
