...
Version 0.26.32 adds SessionSifu-style animated tray activity beside the stable
TuxInDrive logo, a larger adjacent error indicator, and aggregate job-state
handling so concurrent work and outstanding failures remain visible. Version
...
The following controls are enforced in 0.26.32:
...
| --- | --- | --- |
| Ubuntu/Debian | `tuxindrive_0.26.32_all.deb` | Signed in-app Debian updates remain supported. |
| Ubuntu/Debian Server | `tuxindrive-server_0.26.32_all.deb` | Separate preview service; explicit enablement, bearer token, and TLS for remote access. |
...
| Windows 10/11 x64 | `TuxInDrive-0.26.32-windows-x64-setup.exe` | Same GTK desktop UI; install WinFsp for streaming drives. |
| macOS 12+ | `TuxInDrive-0.26.32-macos-*.dmg` | Same GTK desktop UI; install macFUSE for streaming drives. |
| Android 8+ | `TuxInDrive-0.26.32-android.apk` | Native phone/tablet UI, SAF folder access and OS-managed background sync. |
...
```bash
sudo apt install ./tuxindrive_0.26.32_all.deb
```
...
```bash
cd ~/Downloads
sudo apt install ./tuxindrive-server_0.26.32_all.deb
```
...
The Debian installers are written to `dist/tuxindrive_0.26.32_all.deb` and `dist/tuxindrive-server_0.26.32_all.deb`. The independently versioned Network Lab build writes `dist/tuxindrive-network-lab_0.26.31+lab5_all.deb`. Windows, macOS and Android artifacts are built by `.github/workflows/platform-packages.yml` on their native build hosts. Durable product packages are attached to the matching GitHub Release; the lab uses its own `network-lab-v*` release. Dedicated signed client channel manifests and package-location pointers live under [`releases/`](releases/README.md).
