...
Version 0.26.23 adds private local filename search across configured
synchronized folders and retains monitor-safe dialogs that open at 92% of the
active monitor's usable work area, without crossing desktop panels or screen edges.
Their inner content no longer creates immediate horizontal clipping and remains
scrollable after resizing. It retains the automatic aggregate bandwidth
protection from 0.26.21, optional-server and Android update-channel hardening
from 0.26.20, and bidirectional integrity repair from 0.26.19.
...
The current 0.26.23 desktop targets Ubuntu 24.04/26.04, Debian 12/13, Windows 10/11 x64 and macOS 12+; Android 8+ uses its native mobile interface. The main window remains freely resizable; settings and other dialogs open at a monitor-safe maximum and keep oversized controls reachable through local scrolling. **Nordic Glass**, **Bento Cloud**, and **Midnight Sync** are persistent visual designs. Folder grouping/reordering changes only interface metadata, while explicit offline/online-only controls, streaming mounts, GitHub synchronization, searchable offline help, private cross-folder filename search, six-language localization and functional Nautilus badges preserve their established behavior.
...
The following controls are enforced in 0.26.23:
...
| --- | --- | --- |
| Ubuntu/Debian | `tuxindrive_0.26.23_all.deb` | Signed in-app Debian updates remain supported. |
| Ubuntu/Debian Server | `tuxindrive-server_0.26.23_all.deb` | Separate preview service; explicit enablement, bearer token, and TLS for remote access. |
| Windows 10/11 x64 | `TuxInDrive-0.26.23-windows-x64-setup.exe` | Same GTK desktop UI; install WinFsp for streaming drives. |
| macOS 12+ | `TuxInDrive-0.26.23-macos-*.dmg` | Same GTK desktop UI; install macFUSE for streaming drives. |
| Android 8+ | `TuxInDrive-0.26.23-android.apk` | Native phone/tablet UI, SAF folder access and OS-managed background sync. |
...
The Debian installers are written to `dist/tuxindrive_0.26.23_all.deb` and `dist/tuxindrive-server_0.26.23_all.deb`. Windows, macOS and Android artifacts are built by `.github/workflows/platform-packages.yml` on their native build hosts. Durable packages are attached to the matching GitHub Release; dedicated signed client channel manifests and package-location pointers live under [`releases/`](releases/README.md).
