...
Version 0.26.33 makes the animated tray activity indicator exactly three times
its previous source diameter while keeping the stable TuxInDrive logo and the
separate error state. Version 0.26.32 added SessionSifu-style animated tray
...
The following controls are enforced in 0.26.33:
...
| --- | --- | --- |
| Ubuntu/Debian | `tuxindrive_0.26.33_all.deb` | Signed in-app Debian updates remain supported. |
| Ubuntu/Debian Server | `tuxindrive-server_0.26.33_all.deb` | Separate preview service; explicit enablement, bearer token, and TLS for remote access. |
...
| Windows 10/11 x64 | `TuxInDrive-0.26.33-windows-x64-setup.exe` | Same GTK desktop UI; install WinFsp for streaming drives. |
| macOS 12+ | `TuxInDrive-0.26.33-macos-*.dmg` | Same GTK desktop UI; install macFUSE for streaming drives. |
| Android 8+ | `TuxInDrive-0.26.33-android.apk` | Native phone/tablet UI, SAF folder access and OS-managed background sync. |
...
```bash
sudo apt install ./tuxindrive_0.26.33_all.deb
```
...
```bash
cd ~/Downloads
sudo apt install ./tuxindrive-server_0.26.33_all.deb
```
...
The Debian installers are written to `dist/tuxindrive_0.26.33_all.deb` and `dist/tuxindrive-server_0.26.33_all.deb`. The independently versioned Network Lab build writes `dist/tuxindrive-network-lab_0.26.31+lab5_all.deb`. Windows, macOS and Android artifacts are built by `.github/workflows/platform-packages.yml` on their native build hosts. Durable product packages are attached to the matching GitHub Release; the lab uses its own `network-lab-v*` release. Dedicated signed client channel manifests and package-location pointers live under [`releases/`](releases/README.md).
