...
- The panel is populated when the main window becomes visible and continues to
  refresh every five seconds; disabling it still removes the timer to preserve
  the 0.26.36 idle-power improvements.
...
- Cancellable private indexed search with reliably closed database handles.
- Interruptible realtime provider scans and non-blocking files-on-demand mount
  startup/recovery.
- Bounded newest-first audit and recovery-history reading.
- Cached server tenant accounting with periodic expiry maintenance.
- Transactional Android SQLite metadata, one-pass mirror enumeration and a
  documented 250,000-entry safety ceiling.
