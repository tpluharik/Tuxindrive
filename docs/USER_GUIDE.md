...
searchable. Exclusion rules and symbolic links are honored. Typing a new query
cancels the previous database operation, so rapidly refining a search does not
leave stale full-index work queued in the background.

Files-on-demand drives are intentionally not traversed: walking their FUSE
directory tree could contact the provider while the application otherwise
appears idle. Fully synchronized local folders inside the same account are
indexed normally. A single configured root is capped at 250,000 entries; if it
...
On the official Android sideload build, **Settings → Automatic app updates** is
enabled by default. TuxInDrive checks the signed Android channel periodically,
only downloads while a suitable network is available and the battery is not
low, verifies the APK, and shows a notification when it is ready. Tap the
notification to open Android's installer and approve the update. Android may
first ask you to allow installs from TuxInDrive. This permission and the final
installation confirmation are controlled by Android and are never bypassed.
...
Select `+` or **Connect account**, then choose Google Drive, Microsoft OneDrive, Dropbox, Box, pCloud, MEGA, Proton Drive, Nextcloud, S3-compatible storage, WebDAV, or SFTP.
...
- The account card shows the display name, provider/state, and identity on
  separate lines. Google Drive, OneDrive, Dropbox, Box and pCloud identities
  are filled in after authorization and at startup; username-based providers
  use their non-secret configured user. GitHub shows its author or repository
  owner. If a provider cannot expose an identity, the card says **Account key**
  instead of presenting that local alias as a login. Only the display identity
  is saved in TuxInDrive metadata; OAuth tokens remain in rclone's encrypted
  configuration and are never placed in a request URL or application log.
- Open the account menu and choose **Rename account** to change only its visible
  sidebar name. Drag the handle at the left of an account card and drop it above
...
- the existing **Configured root**, accurately identified as My Drive, Shared
  with me, a Shared Drive, or a custom Google Drive folder
...
An older job may instead show **Configured Google Drive root — verify My Drive
or Shared Drive**. This is an intentional warning: the old saved label did not
say which Google tree the underlying remote targeted. Choose **Edit**, select
the intended location explicitly, review its folder tree, and save. Changing
the root clears the old baseline so the next two-way run performs a safe
reconciliation.
...
- **Create share link** — for capable providers only, shows a separate warning and creates a public or bearer-style HTTPS link after explicit confirmation. Treat the resulting URL as a secret and revoke it through the provider when it is no longer needed.
...
- **Error details** — immediately show the last reason, reported source
  file/path, local and cloud locations, timestamp, exact job log, and a bounded
  redacted error excerpt. This action does not scan conflicts or contact the
  provider.
...
Tick only reviewed findings and choose a resolution on every selected row:
**Keep both**, **Use local**, **Use cloud/peer**, or **Skip**. Apply the selected
resolutions, or use a bulk authority button for a fully reviewed group. One-sided
files absent from the authoritative side are removed only after a recovery copy
is retained where possible. **Keep both** preserves the local file and installs
the remote version under a dated `tuxindrive-cloud-conflict` name. TuxInDrive
asks for confirmation and repairs only those paths. Run **Verify** again after
repair; a completed transfer is not itself proof that every byte now matches.

### Conflict review center

Select **Conflicts** to open the same graphical per-file resolver filtered to
content mismatches. Choose a resolution for every reviewed item. The default
for a two-sided mismatch is **Keep both**, so neither version is silently
overwritten. Automatic keep-both synchronization can also create dated
`tuxindrive-conflict` copies; inspect both types before removing either copy.
...
New profiles default to **Apply network, battery and schedule policies**. They
pause automatic transfers below 25% battery while unplugged; metered-network
and daily schedule restrictions remain user-selectable. Independently, the default global
...
this device has a dedicated connection. The default reserves 50% of the ceiling
...
- a battery percentage below which transfers pause while AC power is disconnected (default `25`; `0` disables it);
